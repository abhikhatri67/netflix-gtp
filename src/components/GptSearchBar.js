...
const GptSearchBar = () => {
  ...
  const handleGptSearchClick = async () => {
    // Make an API call to GET movies results from GPT.
    const gptQuery =
    ...
    // "Dangal, Lagaan, Barfi, Queen, Andhadhun"
  ...
  return (
    <div className="pt-[35%] md:pt-[10%] flex justify-center">
      <form className="w-[90%] md:w-1/2 bg-black grid grid-cols-12" onSubmit={e => e.preventDefault()}>
        ...
        <button className="py-2 px-4 m-4 bg-red-700 text-white col-span-3 rounded-lg" onClick={handleGptSearchClick}>
          {lang[langKey].search}
        </button>
      </form>
    </div>
